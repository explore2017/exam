...
class QueryClass extends React.Component {
  ...
  getSearchData(value){
     ...
     this.state.initialData.map((item)=>{
       ...
       if(item.name==value){
        ...
        if((item.classNo.toString()).indexOf(value)!=-1){
          newData.push(item);
        }
       } else if(this.searchKey==3){ 
         if(item.subjectName!==null){
          if((item.subjectName.toString()).indexOf(value)!=-1){        
            newData.push(item);
          }
         }             
       }  
  ...
  deleteClass(record){
    confirm({
      ...
      cancelText : '取消',
      onOk:()=>{
        DELETE(URL.delete_class,record.id)
        .then((res)=>{
          if(res.status==0)
          { 
           this.getClassDate();
    ...
    const columns = [{
      ...
      render: ( record) => (
        <span>
          ...
          <Link
              to={`/main/class_manage/query_class/detail/${record.id}/${record.name}/${record.teacherName}`}
            >查看班级详情</Link>
          </Button>
        </span>
      ),
